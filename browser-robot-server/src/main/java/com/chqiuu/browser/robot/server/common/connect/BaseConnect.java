package com.chqiuu.browser.robot.server.common.connect;
...
public abstract class BaseConnect {
    ...
    public int update(String sql) {
        ...
        try {
            ...
            e.printStackTrace();
            log.error("Message: {} ,sql: {}", e.getMessage(), sql);
        } finally {
            close(conn, stmt);
        }
        return intReturn;
     ...
     * @return 执行结果状态码
     */
    public int update(String sqlTemplate, List<String> values) {
        int resultCode = -1;
        Connection conn = null;
        PreparedStatement preparedStatement = null;
        try {
            conn = getDataSource().getConnection();
            preparedStatement = conn.prepareStatement(sqlTemplate);
            if (values != null && values.size() > 0) {
                setSqlValues(preparedStatement, values);
            }
            log.debug("executeUpdate SQL: {} {}", sqlTemplate, values);
            resultCode = preparedStatement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
            log.error("Message: {} ,sql: {}", e.getMessage(), sqlTemplate);
        } finally {
     ...
     * 向预定义模板填充键值对
     */
    private void setSqlValues(PreparedStatement preparedStatement, List<String> values) {
        for (int i = 0; i < values.size(); i++) {
            try {
                preparedStatement.setObject(i + 1, values.get(i));
            } catch (SQLException e) {
                e.printStackTrace();
                log.error("Message: {} ,values: {}", e.getMessage(), values);
            }
        }
    }
